...
def _get_qual_prefetch_path():
    """今日付きの定性分析プリフェッチファイルパスを返す"""
    import datetime as _dt
    today = _dt.date.today().strftime("%Y%m%d")
    return Path(__file__).resolve().parent.parent / "cache" / f"qual_prefetch_{today}.json"
...
def _load_qual_prefetch(path) -> Optional[QualReport]:
    ...
    try:
        ...
        logger.warning(f"Qual prefetch load error: {e}")
        return None

    logger.info(f"📂 Qual prefetch found: {path.name} — skipping live scraping")
    report = QualReport()

    # ── Forum posts ──
    forum_raw = raw.get("forum_posts", [])
    posts = []
    for p in forum_raw:
        posts.append(ForumPost(
            date=p.get("d", ""),
            ...
            yes_count=p.get("y", 0),
            no_count=p.get("x", 0),
            engagement=p.get("y", 0) + p.get("x", 0),
        ))
    logger.info(f"  掲示板: {len(posts)} 件（prefetch）")

    forum_analysis = _analyze_forum_posts(posts)
    report.yahoo_bbs.post_count = len(posts)
...
class QualitativeAnalyzer:
    def analyze(self) -> QualReport:
        # ── Chrome/WebSearch プリフェッチがあれば優先使用 ──
        prefetch_path = _get_qual_prefetch_path()
        if prefetch_path.exists():
            result = _load_qual_prefetch(prefetch_path)
            if result:
                return result
